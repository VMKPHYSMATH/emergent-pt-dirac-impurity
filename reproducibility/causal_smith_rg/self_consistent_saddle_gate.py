...
def read_csv_typed(path: Path) -> list[dict[str, Any]]:
    ...
    integer_fields = {"converged", "coarse_function_calls", "refined_function_calls"}
...
def run(out: Path, p: SaddleParams = P, reuse_seed_rows: bool = False) -> dict[str, Any]:
    ...
    script_relative = os.path.relpath(script_path, start=out.resolve())
    checksum_lines.append(
        f"{sha256(script_path)}  {script_relative}"
    )
